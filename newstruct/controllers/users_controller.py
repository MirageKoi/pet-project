...
from models.user import User, UserCreateValidation, UserList
...
class UsersController(object):
    ...
    async def user_detail(self, request: Request) -> Response:
        ...
        return web.Response(
            body=record.model_dump_json(), content_type="application/json"
        )

    async def user_create(self, request: Request) -> Response:
        body = await request.post()
        new_user = UserCreateValidation(
            email=body.get("email", None), age=body.get("age", None)
        )

        record = await self.users_service.create(new_user=new_user)

        return web.Response(
            body=record.model_dump_json(), content_type="applicaiton/json"
        )

    async def user_update(self, request: Request) -> Response[User]:
        user_id = int(request.match_info.get("id"))
        kwargs = await request.post()
        updated_user = await self.users_service.update(user_id=user_id, kwargs=kwargs)

        return web.Response(
            body=updated_user.model_dump_json(), content_type="applicatoin/json"
        )
